...
// Goroutine与线程：
// 1. 栈：
// 每个OS线程都有一个固定大小的栈内存（通常2MB），这块区域用来保存其他函数调用期间那些正在执行或临时暂停的函数中的局部变量；
// goroutine在声明周期开始时有一个很小的栈（通常2KB），goroutine的栈也用于存放那些正在执行或临时暂停的函数中的变量。
// 但goroutine的栈不固定，可以按需增大和缩小。
// 2. 调度：
// 线程由OS内核调度，每隔几毫秒，一个硬件时钟中断发到CPU，CPU调用调度器（一个内核函数）；
...
// Go运行时有自己的调度器（m:n调度技术，可以复用/调度m个goroutine到n个线程），该调度器只需要关心单个Go程序的goroutine调度问题；
// Go调度器由特定的Go语言结构来触发，如：当一个goroutine调用time.Sleep或被channel阻塞或lock操作时，调度器就会将该goroutine设为休眠并运行其他goroutine直到前一个可唤醒。
// 3. 标识：
// 线程大部分都有一个独特的标识，通常是整数或指针，以这个特征可以构建一个线程的局部存储；
...
func TestGoroutineBasic(t *testing.T) {
	// NumCPU() 获取本机的CPU核数
	num := runtime.NumCPU()
	fmt.Printf("本机CPU个数: %v\n", num)

	// runtime.GOMAXPROCS()
	// 1. 设置用来并行计算的CPU核数的最大值，返回上一次设置的最大值
	...
	go func() {
		for i := 0; i < 10; i++ {
			...
			fmt.Printf("go func() 执行 %v 次\n", i)
			if i == 5 {
				// 主动让出当前go程所占用的cpu时间片，当再次获得cpu时，从让出的位置继续恢复执行
				runtime.Gosched()
			} else if i == 8 {
				// 结束当前go程，执行所有defer
				runtime.Goexit()
